...
# Reference: https://github.com/musikalkemist/generativemusicaicourse/blob/main/12.%20Melody%20generation%20with%20Markov%20chains/Code/markovchain.py
"""
The following class implements a basic Markov Chain model for melody generation.
The idea is to construct a graph where each note represents a node. Further a path from note a 
to b represents the probability of going from note a to b, given that we are currently
at note a. In other words, given the Markov Chain model and an initial note, we can generate
a melody consisting of a sequence of notes through a random walk along the paths within the graph.
...
class MarkovChainMelodyGenerator:
    ...
    def generate(self, length):
        ...
        while time < length:
            ...
            time += melody[-1][1]
        melody[-1] = (melody[-1][0], length - (time - melody[-1][1]))
...
def create_training_data():
    ...
    return [
        ...
        note.Note("G5", quarterLength=0.5),
        note.Note("A5", quarterLength=0.5),
        note.Note("A5", quarterLength=0.5),
...
def create_training_data_ode_of_joy():

    """
    Creates a list of sample training notes for the melody of "Ode of Joy."
    ...
    return [
        ...
        note.Note("D5", quarterLength=1),
        note.Note("E5", quarterLength=1),
        note.Note("E5", quarterLength=1.5),
        note.Note("D5", quarterLength=0.5),
        ...
        note.Note("D5", quarterLength=1),
        note.Note("E5", quarterLength=1),
        note.Note("D5", quarterLength=1.5),
        note.Note("C5", quarterLength=0.5),
        note.Note("C5", quarterLength=2),
    ]

def create_training_data_3():
    ...
    return [
        note.Note("G4", quarterLength=0.75),
        note.Note("G4", quarterLength=0.25),
        note.Note("G4", quarterLength=0.75),
        note.Note("F4", quarterLength=0.25),
        note.Note("E4", quarterLength=0.75),
        note.Note("G4", quarterLength=0.25),
        note.Note("C5", quarterLength=0.75),
        note.Note("D5", quarterLength=0.25),
        note.Note("E5", quarterLength=0.75),
        note.Note("E5", quarterLength=0.25),
        note.Note("E5", quarterLength=0.75),
        note.Note("D5", quarterLength=0.25),
        note.Note("C5", quarterLength=1),
        note.Note("C5", quarterLength=0.75),
        note.Note("B4", quarterLength=0.25),
        note.Note("A4", quarterLength=0.75),
        note.Note("A4", quarterLength=0.25),
        note.Note("A4", quarterLength=0.75),
        note.Note("B4", quarterLength=0.25),
        note.Note("C5", quarterLength=0.75),
        note.Note("B4", quarterLength=0.25),
        ...
        note.Note("C5", quarterLength=4),

    ]


def create_training_data_jasmine():


    """
    Creates a list of sample training notes for the melody of "Mo Li Hua" (Jasmine)
